...
import { fetchMetaAccounts, updateMetaAccount } from '../services/supabaseService';
...
export const AccountsConfig: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await updateMetaAccount(accountId, {
                account_name: accountName,
                franchise_id: franqueado,
                display_name: nomeAjustado || undefined
            });
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Ajuste de visibilidade e nomes customizados para Contas da Meta (sincronizadas automaticamente).</p>
                </div>
                <button
            ...
            <div className="bg-white rounded-3xl p-6 shadow-lg shadow-indigo-500/5 border border-slate-100">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 uppercase">Franchise ID *</label>
                        <input
                            type="text"
                            value={franqueado}
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-100">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Franquia ID</th>
                                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Nome Ajustado</th>
                                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Último Sync</th>
                            </tr>
                        </thead>
                            ...
                            {loading ? (
                                ...
                                accounts.map((acc) => (
                                    <tr key={acc.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-sm font-medium text-indigo-600 bg-indigo-50/50 rounded-lg">{acc.franchise_id || '-'}</td>
                                        <td className="px-6 py-4 text-sm text-slate-600 italic">{acc.display_name || '-'}</td>
                                        <td className="px-6 py-4 text-sm text-slate-500">
                                            {acc.last_sync ? new Date(acc.last_sync).toLocaleDateString() : 'Nunca'}
                                        </td>
                                    </tr>
                                ))
                            )}
