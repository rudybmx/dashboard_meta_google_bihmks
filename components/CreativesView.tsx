...
const aggregateCreatives = (data: CampaignData[]) => {
  ...
  data.forEach(row => {
    ...
    if (!groups[id]) {
      groups[id] = {
        id,
        account_name: (row as any).nome_conta || (row as any).account_name || 'Conta Desconhecida',
    ...
    groups[id].leads += (Number((row as any).leads || row.leads_total || 0));
...
export const CreativesView: React.FC<Props> = ({ data }) => {
  ...
  const getBadge = (ad: any) => {
    const isWinner = ad.leads > 3 && ad.cpl > 0 && ad.cpl <= (avgCpl * 0.8);
    if (isWinner) {
          ...
          {sortedCreatives.map((ad) => (
            <div key={ad.id} className="group bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col">
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <p className="text-[10px] text-slate-500 font-medium truncate mb-0.5">{ad.account_name}</p>
                <p className="text-[10px] text-slate-400 font-mono truncate mb-1">{ad.ad_name}</p>
