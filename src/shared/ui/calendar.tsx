...
import { Material } from "@/src/shared/ui/material-1";
import { Input } from "@/src/shared/ui/input";
import { Select } from "@/src/shared/ui/select-1";
import { formatInTimeZone, fromZonedTime } from "date-fns-tz";
import { useClickOutside } from "@/src/shared/ui/use-click-outside";
        ...
        <>
          ...
          {compact && (
            <div
              ...
              onClick={() => { setIsOpen(false); setTempRange({ start: value?.start || null, end: value?.end || null }); }}
            />
          )}

          {/* ── MOBILE: bottom-sheet (compact mode) ── */}
          <div
            ref={compact ? calendarRef : undefined}
            className={twMerge(clsx(
              "md:hidden fixed inset-x-0 bottom-0 z-[9999] bg-white border-t border-slate-200 shadow-2xl rounded-t-2xl flex flex-col gap-4 p-4",
              !compact && "hidden"
            ))}
          >
            {/* Drag pill */}
            <div className="mx-auto w-10 h-1 rounded-full bg-slate-200 -mt-1 mb-1" />

            {/* Date display */}
            <div className="flex gap-3">
              <div className="flex-1 text-center">
                <p className="text-[10px] text-slate-400 mb-1 uppercase tracking-wide">Início</p>
                <div className="h-9 flex items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-sm font-medium text-slate-700">
                  {startDateStr || <span className="text-slate-300">dd/mm/aaaa</span>}
                </div>
              </div>
              <div className="flex-1 text-center">
                <p className="text-[10px] text-slate-400 mb-1 uppercase tracking-wide">Fim</p>
                <div className="h-9 flex items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-sm font-medium text-slate-700">
                  {endDateStr || <span className="text-slate-300">dd/mm/aaaa</span>}
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1">
              {typeRelativeTimes.filter(p => p.id !== 'custom').map((preset) => (
                <button
                  key={preset.id}
            ...
            <div className="flex justify-center">
              <div className="relative w-[280px]">
                <button onClick={prevMonth} className="absolute left-0 top-0 z-10 p-1.5 hover:bg-slate-100 rounded-lg transition-colors">
                  <ArrowLeftIcon />
                </button>
                <button onClick={nextMonth} className="absolute right-0 top-0 z-10 p-1.5 hover:bg-slate-100 rounded-lg transition-colors">
                  <ArrowRightIcon />
                </button>
            ...
            <div className="flex gap-3 pt-2 border-t border-slate-100">
              <button onClick={cancelFilter} className="flex-1 h-11 rounded-xl text-sm font-semibold border border-indigo-600 text-indigo-600 hover:bg-indigo-50 transition-colors">
                Cancelar
              </button>
              <button onClick={applyFilter} className="flex-1 h-11 rounded-xl text-sm font-semibold bg-indigo-600 text-white hover:bg-indigo-700 transition-colors shadow-sm">
                Filtrar
              </button>
            </div>
          </div>
          ...
          <div
            ref={!compact ? calendarRef : undefined}
            className={twMerge(clsx(
              "hidden md:flex absolute mt-2 p-5 bg-white border border-slate-200 shadow-2xl rounded-2xl z-[9999] flex-col gap-5",
              popoverAlignment === "end" ? "right-0" : "left-0"
            ))}
          ...
          >
            ...
            <div className="flex gap-4 items-end">
              <div className="flex-1">
                ...
                <input type="text" value={startDateStr} readOnly placeholder="dd/mm/aaaa" className="w-full h-10 px-4 rounded-lg border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-center font-medium bg-slate-50/50" />
              </div>
              <div className="flex-1">
                <label className="text-xs text-slate-500 mb-1.5 block font-medium">Fim do período</label>
                <input type="text" value={endDateStr} readOnly placeholder="dd/mm/aaaa" className="w-full h-10 px-4 rounded-lg border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-center font-medium bg-slate-50/50" />
              </div>
              ...
              <div className="flex flex-col gap-6">
                <div className="flex gap-6">
                  <div className="relative">
                    <button onClick={prevMonth} className="absolute left-0 top-0 z-10 p-1 hover:bg-slate-100 rounded-lg transition-colors"><ArrowLeftIcon /></button>
                    {renderMonth(currentDate)}
                  </div>
                  <div className="relative">
                    <button onClick={nextMonth} className="absolute right-0 top-0 z-10 p-1 hover:bg-slate-100 rounded-lg transition-colors"><ArrowRightIcon /></button>
                ...
                {typeRelativeTimes.map((preset) => (
                  <button
                    ...
                    className={clsx(
                      ...
                      selectedPresetId === preset.id ? "bg-indigo-50 text-indigo-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                    )}
                  >
                    {preset.text}
                  </button>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
              <button onClick={cancelFilter} className="px-6 h-10 rounded-lg text-sm font-semibold border border-indigo-600 text-indigo-600 hover:bg-indigo-50 transition-colors">Cancelar</button>
              <button onClick={applyFilter} className="px-8 h-10 rounded-lg text-sm font-semibold bg-indigo-600 text-white hover:bg-indigo-700 transition-colors shadow-sm">Filtrar</button>
            </div>
          </div>
        </>
      )}
